...
4. Behavior:
   - Lead **opened the first email but did not reply**.
   - Focus on reinforcing value or presenting a subtle new angle.
   - Avoid referencing previous emails explicitly.
...
"""
    ...
    user_prompt = f"""
You’re writing a follow-up email to a prospect who opened but did not reply to our first email.
...
def get_rotated_template(last_template_type=None) -> Optional[Template]:
    templates = list(Template.objects.filter(template_type="follow_up", is_active=True))

    if not templates:
        print("[WARN] No active follow-up templates found.")
        return None
...
# -----------------------------
# Fetch recently opened leads
# -----------------------------
def fetch_recently_opened():
    now = timezone.now()
    threshold = now - timedelta(hours=FOLLOWUP_HOURS_AFTER_OPEN)
    ...
    print(f"[INFO] Found {leads.count()} recently opened leads.")
    return leads
...
def create_followups():
    opened_leads = fetch_recently_opened()
    if not opened_leads.exists():
        print("[INFO] No leads to create follow-ups for.")
        return

    for ot in opened_leads:
        lead = ot.lead

        last_followup = FollowUp.objects.filter(lead=lead).order_by('-created_at').first()
        last_template_type = last_followup.template_type if last_followup else None
        ...
        followup = FollowUp.objects.create(
            lead=lead,
            parent_email=ot,  # correct field
            followup_number=(last_followup.followup_number + 1) if last_followup else 1,
        ...
        print(f"[SUCCESS] Follow-up #{followup.followup_number} created for {lead.email}")
...
if __name__ == "__main__":
    print("[INFO] Starting follow-up generation...")
    create_followups()
    print("[INFO] Done.")
