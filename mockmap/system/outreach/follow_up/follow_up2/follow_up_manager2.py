...
# Config
# -----------------------------
SECOND_FOLLOWUP_HOURS_AFTER_FIRST = 72  # 3 days after first follow-up
...
3. Tone & Style:
   - Casual, human, and approachable.
   - Short and digestible: 3 sentences max.
   - Natural, human-like; avoid over-formality and jargon.
   - Use spintax if the template contains it for variation.

4. Behavior:
   - Assume the lead **opened the first follow-up but did not reply**.
   - Reaffirm value or provide a subtle new angle.
   - Avoid referencing prior emails explicitly.
   - Focus on quick value and low-friction CTA.

5. Content Guidelines:
   - Subject: lowercase, curiosity-driven, ≤30 characters.
   - Body: compliment → problem → hint solution.
   - Word count: under 70 words.
   - End with a soft, easy-to-reply CTA.

"""

    user_prompt = f"""
You’re writing a 2nd follow-up email to a prospect who opened our first follow-up email but did not reply.
...
def generate_personalized_email(lead, template_content: str, max_retries: int = MAX_GPT_RETRIES):
    ...
    for attempt in range(max_retries):
        try:
            print(f"[INFO] Generating email for {lead.email}, attempt {attempt + 1}")
            ...
            if subject and body:
                print(f"[SUCCESS] GPT email generated for {lead.email}")
                return subject, body
...
def get_rotated_template(last_template_type=None) -> Optional[Template]:
    templates = list(Template.objects.filter(template_type="follow_up_2", is_active=True))
    if not templates:
        print("[WARN] No active 2nd follow-up templates found.")
        return None
...
# -----------------------------
# Fetch first follow-ups eligible for second follow-up
# -----------------------------
def fetch_first_followups_for_second():
    threshold = timezone.now() - timedelta(hours=SECOND_FOLLOWUP_HOURS_AFTER_FIRST)
    ...
    print(f"[INFO] Found {followups.count()} first follow-ups to retarget")
    return followups

# -----------------------------
# CREATE SECOND FOLLOW-UPS
# -----------------------------
def create_second_followups():
    first_followups = fetch_first_followups_for_second()
    if not first_followups.exists():
        print("[INFO] No leads to create second follow-ups for.")
        return
    ...
    for first in first_followups:
        ...
        if not subject or not body:
            ...
            continue
        parent_email_tracking = first.parent_email  # OutreachTracking instance

        followup = FollowUp.objects.create(
            lead=lead,
            parent_email=parent_email_tracking,  # points to first follow-up
            followup_number=2,
            template=template,
        ...
        print(f"[SUCCESS] 2nd follow-up created for {lead.email}")

# -----------------------------
# Run
# -----------------------------
if __name__ == "__main__":
    print("[INFO] Starting 2nd follow-up generation...")
